 ...
 * where long double is the same as double, ftl_olm_nexttoward() is an alias
 * for ftl_olm_nextafter(), so we don't use this routine.
 */
...
ftl_olm_nexttoward(double x, long double y)
{
	union IEEEl2bits uy;
	volatile double t;
	int32_t hx,ix;
