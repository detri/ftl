 ...
 * amount of work relative to just calling ftl_olm_sin/ftl_olm_cos(x)
 * separately
 *
 * Special cases:
 *      Let trig be any of ftl_olm_sin, ftl_olm_cos, or ftl_olm_tan.
 *      ftl_olm_sincos(+-INF, s, c)  is NaN, with signals;
 *      ftl_olm_sincos(NaN, s, c)    is that NaN;
 */
...
/* Constants used in polynomial approximation of ftl_olm_sin/ftl_olm_cos */
static const double
...
{
    /* Inline calculation of ftl_olm_sin/ftl_olm_cos, as we can save
    ...
    /* ftl_olm_cos-specific computation; equivalent to calling
     ftl_olm___kernel_cos(x,y) and storing in k_c*/
    ...
    *k_c = v + (((one-v)-hz) + (z*r-x*y));
    ...
    /* ftl_olm_sin-specific computation; equivalent to calling
    ftl_olm___kernel_sin(x,y,1) and storing in k_s*/
...
OLM_DLLEXPORT void
ftl_olm_sincos(double x, double * s, double * c)
{
    double y[2];
    int32_t ix;
    ...
    ix &= 0x7fffffff;
    if(ix <= 0x3fe921fb) {
        /* Check for small x for ftl_olm_sin and ftl_olm_cos */
        if(ix<0x3e46a09e) {
        ...
        /* ftl_olm_sincos(Inf or NaN) is NaN */
        *s = x-x;
        *c = x-x;
    }
    ...
    /*argument reduction needed*/
        ...
        int32_t n = ftl_olm___ieee754_rem_pio2(x,y);
        __kernel_sincos( y[0], y[1], 1, &k_s, &k_c );

        /* Figure out permutation of ftl_olm_sin/ftl_olm_cos outputs to true outputs */
        switch(n&3) {
            case 0:
                *c =  k_c;
                *s =  k_s;
...
#if (LDBL_MANT_DIG == 53)
openlibm_weak_reference(ftl_olm_sincos, ftl_olm_sincosl);
#endif
