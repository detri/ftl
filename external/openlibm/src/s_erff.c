 ...
 *  |(ftl_olm_erf(x) - x)/x - p(x)/q(x)| < 2**-31.
 */
 ...
 * |(ftl_olm_erf(x) - erx) - p(x)/q(x)| < 2**-36.
 */
erx  =  8.42697144e-01F, /*  0x1.af7600p-1.  ftl_olm_erf(1) rounded to 16 bits. */
 ...
 * |ftl_olm_log(x*ftl_olm_erfc(x)) + x**2 + 0.5625 - r(x)/s(x)| < 2**-30
 */
 ...
 * |ftl_olm_log(x*ftl_olm_erfc(x)) + x**2 + 0.5625 - r(x)/s(x)| < 2**-42
 */
...
OLM_DLLEXPORT float
ftl_olm_erff(float x)
{
	int32_t hx,ix,i;
	float R,S,P,Q,s,y,z,r;
	GET_FLOAT_WORD(hx,x);
	ix = hx&0x7fffffff;
	if(ix>=0x7f800000) {		/* ftl_olm_erf(ftl_olm_nan)=ftl_olm_nan */
	    i = ((u_int32_t)hx>>31)<<1;
	    return (float)(1-i)+one/x;	/* ftl_olm_erf(+-inf)=+-1 */
	}
	...
	if(ix< 0x4036DB6E) {	/* |x| < 1/0.35 */
        ...
        r  = ftl_olm_expf(-z*z-0.5625F)*ftl_olm_expf((z-x)*(z+x)+R/S);
	if(hx>=0) return one-r/x; else return  r/x-one;
}

OLM_DLLEXPORT float
ftl_olm_erfcf(float x)
{
	int32_t hx,ix;
	float R,S,P,Q,s,y,z,r;
	GET_FLOAT_WORD(hx,x);
	ix = hx&0x7fffffff;
	if(ix>=0x7f800000) {			/* ftl_olm_erfc(ftl_olm_nan)=ftl_olm_nan */
