...
/* long double version of ftl_olm_hypot().  See e_hypot.c for most comments. */
...
#define	DESW(ftl_olm_exp)	(ftl_olm_exp)		/* delta expsign word */
#define	ESW(ftl_olm_exp)	(MAX_EXP - 1 + (ftl_olm_exp))	/* expsign word */
...
{
	long double a=x,b=y,t1,t2,ftl_olm_y1,y2,w;
	int32_t j,k,ha,hb;
	...
	if(ha > ESW(MAX_EXP/2-12)) {	/* a>2**(MAX_EXP/2-12) */
	   if(ha >= ESW(MAX_EXP)) {	/* Inf or NaN */
	       ...
	       w = ftl_olm_fabsl(x+0.0)-ftl_olm_fabsl(y+0.0);
	...
	if (w>b) {
	    ...
	    ftl_olm_y1 = b;
	    union IEEEl2bits uv;
	    uv.e = ftl_olm_y1; uv.bits.manl = 0; ftl_olm_y1 = uv.e;
	    y2 = b - ftl_olm_y1;
	    t1 = a;
	    uv.e = t1; uv.bits.manl = 0; t1 = uv.e;
	    t2 = a - t1;
	    w  = ftl_olm_sqrtl(t1*ftl_olm_y1-(w*(-w)-(t1*y2+t2*b)));
	}
	if(k!=0) {
	    u_int32_t high;
	    t1 = 1.0;
