 ...
 *
 * kernel function:
 *	ftl_olm___kernel_tan		... tangent function on [-pi/4,pi/4]
...
ftl_olm_tan(double x)
{
	...
	if(ix <= 0x3fe921fb) {
	    ...
	    return ftl_olm___kernel_tan(x,z,1);
	}
    ...
    /* argument reduction needed */
	    ...
	    return ftl_olm___kernel_tan(y[0],y[1],1-((n&1)<<1)); /*   1 -- n even
							-1 -- n odd */
	}
}

#if (LDBL_MANT_DIG == 53)
openlibm_weak_reference(ftl_olm_tan, ftl_olm_tanl);
#endif
