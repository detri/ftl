...
#pragma intrinsic(_InterlockedIncrement)
    ...
    ftl_file* standard_stream(unsigned index)
    {
        ...
        {
            stream.handle = index == 0
                                ? native_standard_input()
                                : index == 1
                                      ? native_standard_output()
    ...
    bool ensure_wide_locale(ftl_file* stream)
    {
        ...
        stream->wide_locale =
                ftl_locale_runtime::snapshot_current_ctype();

        if (stream->wide_locale == nullptr)
            return false;
                    ...
                    bool& writable, bool& binary)
    {
        ...
        options.access = update
                             ? native_file_access::read_write
                             : readable
                                   ? native_file_access::read
                                   : native_file_access::write;
        options.append = operation == 'a';
        options.creation = operation == 'r'
                               ? native_file_creation::open_existing
                               : exclusive
                                     ? native_file_creation::create_new
                                     : operation == 'w'
                                           ? native_file_creation::create_always
...
{
    file_type* input_stream() noexcept
    {
        return standard_stream(0);
    }

    file_type* output_stream() noexcept
    {
        return standard_stream(1);
    }

    file_type* error_stream() noexcept
    {
        return standard_stream(2);
    }
    ...
    int read_wide_byte_locked(file_type* stream) noexcept
    {
        ...
        for (;;)
        {
            ...
            const auto decoded =
                    ftl_locale_runtime::decode_wide_restartable(
                        stream->wide_locale, stream->wide_state,
                        &input, &input + 1);
            ...
            /*
             * partial: the byte has been consumed into wide_state.
             * Read another physical byte and resume.
             */
        }
    }
    ...
    int write_wide_byte_locked(int value, file_type* stream) noexcept
    {
        ...
        /*
         * Only commit the conversion state once the corresponding physical
         * bytes were successfully written.
         */
        stream->wide_state = next_state;
        return value;
    }
    ...
    int unget_wide_byte_locked(int value, file_type* stream) noexcept
    {
        ...
        const auto encoded =
                ftl_locale_runtime::encode_wide_restartable(
                    stream->wide_locale, probe_state,
                    static_cast<wchar_t>(value));
        ...
        stream->wide_pushback_bytes =
                static_cast<unsigned>(encoded.produced);
        stream->eof = false;
...
namespace ftl_cstdio_runtime
{
    ...
    int getchar()
    {
        return fgetc(ftl_stdio_runtime::input_stream());
    }
