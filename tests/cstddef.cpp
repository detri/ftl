...
static_assert(tested::to_integer<unsigned>(tested::byte{3} >> 1) == 1);
static_assert(tested::to_integer<unsigned>(tested::byte{1} | tested::byte{2}) == 3);
static_assert(tested::to_integer<unsigned>(tested::byte{3} & tested::byte{1}) == 1);
static_assert(tested::to_integer<unsigned>(tested::byte{3} ^ tested::byte{1}) == 2);
static_assert(tested::to_integer<unsigned>(~tested::byte{0}) == 255);
constexpr auto compound_byte_operations = [] {
    auto value = tested::byte{1};
    value <<= 1;
    value >>= 1;
    value |= tested::byte{2};
    value &= tested::byte{3};
...
static_assert(tested::to_integer<unsigned>(compound_byte_operations) == 2);
