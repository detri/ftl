...
public:
    ...
    constexpr explicit input_test_iterator(
        T* current
    ) noexcept
        : current_(current) {}
...
public:
    ...
    constexpr explicit forward_test_iterator(
        T* current
    ) noexcept
        : current_(current) {}
...
template<class T>
class bidirectional_test_iterator {
    T* current_ = nullptr;
...
public:
    ...
    constexpr bidirectional_test_iterator() =
        default;

    constexpr explicit
    bidirectional_test_iterator(
        T* current
    ) noexcept
        : current_(current) {}
    ...
    friend constexpr bool operator==(
        const bidirectional_test_iterator&,
        const bidirectional_test_iterator&
    ) = default;
};

template<class T>
constexpr T* iterator_address(
    T* iterator
) noexcept {
    return iterator;
}
...
template<class T>
constexpr T* iterator_address(
    const input_test_iterator<T>& iterator
) noexcept {
...
template<class T>
constexpr T* iterator_address(
    const forward_test_iterator<T>& iterator
) noexcept {
    return iterator.base();
}
...
template<class T>
constexpr T* iterator_address(
    const bidirectional_test_iterator<T>&
        iterator
) noexcept {
    return iterator.base();
}
...
struct sized_sentinel {
    ...
    operator-(
        ...
        return
            iterator_address(sentinel.last) -
            iterator_address(iterator);
    }
...
using input_iterator =
    input_test_iterator<int>;

using forward_iterator =
    forward_test_iterator<int>;

using bidirectional_iterator =
    bidirectional_test_iterator<int>;

using common_input_view =
    basic_test_view<input_iterator>;

using non_common_input_view =
    basic_test_view<
        input_iterator,
        unsized_sentinel<input_iterator>
    >;

using common_forward_view =
    basic_test_view<forward_iterator>;
...
using non_common_sized_forward_view =
    basic_test_view<
        forward_iterator,
        sized_sentinel<forward_iterator>
    >;

using common_bidirectional_view =
    basic_test_view<
...
using pair_forward_iterator =
    forward_test_iterator<pair_type>;

using common_pair_view =
    basic_test_view<
        pair_forward_iterator
    >;
...
concept common_mutable_and_const =
    correctly_common<R> &&
    correctly_common<const R>;

template<class R>
concept non_common_mutable_and_const =
    correctly_non_common<R> &&
    correctly_non_common<const R>;
...
static_assert(
    common_mutable_and_const<
        common_input_view
    >
);

static_assert(
    non_common_mutable_and_const<
        non_common_input_view
    >
);
...
static_assert(
    non_common_mutable_and_const<
        non_common_forward_view
    >
);

static_assert(
    tested::ranges::sized_range<
        non_common_sized_forward_view
    >
);
...
static_assert(
    non_common_mutable_and_const<
        non_common_bidirectional_view
    >
);

static_assert(
    tested::ranges::random_access_range<
        non_common_random_access_view
    >
);
...
using common_elements =
    decltype(
        tested::ranges::views::elements<0>(
            tested::declval<
                common_pair_view&
            >()
        )
...
using non_common_elements =
    decltype(
        tested::ranges::views::elements<0>(
            tested::declval<
                non_common_pair_view&
            >()
        )
    );
...
using unsized_common_enumerate =
    decltype(
        tested::ranges::views::enumerate(
            tested::declval<
                common_forward_view&
...
using non_common_sized_enumerate =
    decltype(
        tested::ranges::views::enumerate(
            tested::declval<
                non_common_random_access_view&
...
static_assert(
    correctly_non_common<
        unsized_common_enumerate
    >
);

static_assert(
    correctly_non_common<
        input_common_enumerate
    >
);

static_assert(
    correctly_non_common<
        non_common_sized_enumerate
    >
);
...
using non_common_bidirectional_stride =
    decltype(
        tested::ranges::views::stride(
            tested::declval<
                common_bidirectional_view&
...
static_assert(
    correctly_non_common<
        non_common_bidirectional_stride
    >
);
...
static_assert(
    correctly_common<
        common_adjacent_transform
    >
);

static_assert(
    correctly_non_common<
        non_common_adjacent_transform
    >
);
...
using common_random_access_slide =
    decltype(
        tested::ranges::views::slide(
            tested::declval<
                non_common_random_access_view&
...
static_assert(
    correctly_common<
        common_forward_slide
    >
);

static_assert(
    correctly_common<
        common_random_access_slide
    >
);
...
using non_common_bidirectional_chunk =
    decltype(
        tested::ranges::views::chunk(
            tested::declval<
                common_bidirectional_view&
...
static_assert(
    correctly_common<
        common_forward_chunk
    >
);

static_assert(
    correctly_non_common<
        non_common_bidirectional_chunk
    >
);
...
using common_inner_array =
    common_forward_view[2];

using non_common_inner_array =
    non_common_forward_view[2];
...
using non_common_inner_join =
    decltype(
        tested::ranges::views::join(
            tested::declval<
                non_common_inner_array&
            >()
        )
    );
...
using non_common_outer_join =
    decltype(
        tested::ranges::views::join(
            tested::declval<
                non_common_outer_view&
            >()
        )
    );
...
static_assert(
    correctly_non_common<
        non_common_inner_join
    >
);

static_assert(
    correctly_non_common<
        non_common_outer_join
    >
);
...
using non_bidirectional_common_zip =
    decltype(
        tested::ranges::views::zip(
            tested::declval<
                common_input_view&
...
using bidirectional_unsized_zip =
    decltype(
        tested::ranges::views::zip(
            tested::declval<
                common_bidirectional_view&
...
static_assert(
    correctly_common<
        non_bidirectional_common_zip
    >
);

static_assert(
    correctly_common<
        random_access_sized_zip
    >
);

static_assert(
    correctly_non_common<
        bidirectional_unsized_zip
    >
);

static_assert(
    correctly_non_common<
        mixed_non_common_zip
    >
);
...
using non_common_zip_transform =
    decltype(
        tested::ranges::views::
            zip_transform(
                add_values{},
...
static_assert(
    correctly_common<
        common_zip_transform
    >
);

static_assert(
    correctly_common<
        random_access_zip_transform
    >
);

static_assert(
    correctly_non_common<
        non_common_zip_transform
    >
);
...
static_assert(
    correctly_common<
        common_first_product
    >
);

static_assert(
    correctly_common<
        random_access_first_product
    >
);

static_assert(
    correctly_non_common<
        non_common_first_product
    >
);
...
bool common_end_values_work() {
    ...
    common_input_view input{
        input_iterator(values),
        input_iterator(values + 5)
    };

    common_forward_view forward{
        forward_iterator(values),
        forward_iterator(values + 5)
    };

    common_forward_view short_forward{
        forward_iterator(short_values),
        forward_iterator(
            short_values + 3
        )
    };
