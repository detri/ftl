...
struct move_only_value {
    ...
    constexpr move_only_value&
    operator=(move_only_value&&) = default;
};
...
concept can_form_repeat_view =
    requires {
        typename tested::ranges::repeat_view<
            Value,
            Bound
        >;
    };
...
using unbounded_view =
    tested::ranges::repeat_view<int>;

using signed_bounded_view =
    tested::ranges::repeat_view<
        int,
        int
    >;

using unsigned_bounded_view =
    tested::ranges::repeat_view<
        int,
        unsigned short
    >;
...
static_assert(
    sizeof(unsigned_difference_type) >=
...
static_assert(
    !can_form_repeat_view<int, bool>
);

static_assert(
    !can_form_repeat_view<int, double>
);
...
static_assert(
    !can_repeat_bounded<int, bool>
);

static_assert(
    !can_repeat_bounded<int, double>
);

static_assert(
    tested::default_initializable<
        signed_bounded_view
    >
);
...
constexpr bool repeat_works() {
    ...
    if (
        unsigned_bounded.end() -
        unsigned_bounded.begin() != 2
    )
        return false;

    auto unbounded =
        tested::ranges::views::repeat(4);
    ...
    tested::ranges::repeat_view<
        ...
        int
    > piecewise(
        tested::piecewise_construct,
        tested::tuple<int, int>(2, 3),
        tested::tuple<int>(2)
    );
