...
template<class T>
class random_access_test_iterator {
    T* current_ = nullptr;
...
public:
    ...
    constexpr explicit random_access_test_iterator(
        T* current
    ) noexcept
        : current_(current) {}
    ...
    friend constexpr bool operator<(
        const random_access_test_iterator& left,
        const random_access_test_iterator& right
    ) noexcept {
        return left.current_ < right.current_;
    }
...
using input_view =
    basic_test_view<
        input_test_iterator<T>
    >;

template<class T>
using forward_view =
    basic_test_view<
        forward_test_iterator<T>
    >;

template<class T>
using bidirectional_view =
    basic_test_view<
        bidirectional_test_iterator<T>
    >;

template<class T>
using random_access_view =
    basic_test_view<
        random_access_test_iterator<T>
    >;

template<class T>
using contiguous_view =
    basic_test_view<T*>;
...
concept exactly_input_range =
    tested::ranges::input_range<R> &&
    !tested::ranges::forward_range<R>;

template<class R>
concept exactly_forward_range =
    tested::ranges::forward_range<R> &&
    !tested::ranges::bidirectional_range<R>;

template<class R>
concept exactly_bidirectional_range =
    tested::ranges::bidirectional_range<R> &&
    !tested::ranges::random_access_range<R>;

template<class R>
concept exactly_random_access_range =
    tested::ranges::random_access_range<R> &&
    !tested::ranges::contiguous_range<R>;

template<class R>
concept exactly_contiguous_range =
    tested::ranges::contiguous_range<R>;
...
consteval bool member_category_is() {
    if constexpr (
        has_member_iterator_category<
            Iterator
        >
    ) {
        return tested::same_as<
            typename Iterator::
                iterator_category,
            Category
        >;
...
consteval bool range_member_category_is() {
    return member_category_is<
...
struct make_pair_value {
    constexpr tested::pair<int, int>
    operator()(int value) const noexcept {
        return {
            value,
            value
        };
...
using int_bidirectional =
    bidirectional_view<int>;

using int_random_access =
    random_access_view<int>;

using int_contiguous =
    contiguous_view<int>;

struct make_forward_view {
    constexpr int_forward operator()(
        int&
    ) const noexcept {
        return {};
    }
...
static_assert(
    exactly_input_range<int_input>
);

static_assert(
    exactly_forward_range<int_forward>
);
...
static_assert(
    exactly_forward_range<
        transform_forward_value
    >
);

static_assert(
    exactly_bidirectional_range<
        transform_bidirectional_reference
    >
);

static_assert(
    exactly_random_access_range<
        transform_random_value
    >
);

static_assert(
    exactly_random_access_range<
        transform_contiguous_reference
    >
);
...
using reverse_bidirectional =
    tested::ranges::reverse_view<
        int_bidirectional
    >;

using reverse_random =
    tested::ranges::reverse_view<
        int_random_access
    >;

using reverse_contiguous =
    tested::ranges::reverse_view<
        int_contiguous
    >;
...
using common_input =
    tested::ranges::common_view<
        non_common_input_view<int>
    >;

using common_forward =
    tested::ranges::common_view<
        non_common_forward_view<int>
    >;

using common_random =
    tested::ranges::common_view<
        non_common_random_access_view<int>
    >;
...
using pair_forward =
    forward_view<pair_type>;

using pair_random =
    random_access_view<pair_type>;

using pair_contiguous =
    contiguous_view<pair_type>;

using elements_forward =
    tested::ranges::elements_view<
        pair_forward,
        0
    >;

using elements_random =
    tested::ranges::elements_view<
        pair_random,
        0
    >;

using elements_contiguous =
    tested::ranges::elements_view<
        pair_contiguous,
        0
    >;
...
using elements_prvalue =
    tested::ranges::elements_view<
        pair_value_transform,
        0
    >;
...
using as_const_input =
    tested::ranges::as_const_view<
        int_input
    >;

using as_const_forward =
    tested::ranges::as_const_view<
        int_forward
    >;

using as_const_contiguous =
    tested::ranges::as_const_view<
        int_contiguous
    >;
...
using stride_input =
    tested::ranges::stride_view<
        int_input
    >;

using stride_forward =
    tested::ranges::stride_view<
        int_forward
    >;

using stride_bidirectional =
    tested::ranges::stride_view<
        int_bidirectional
    >;
...
using enumerate_input =
    tested::ranges::enumerate_view<
        int_input
    >;

using enumerate_forward =
    tested::ranges::enumerate_view<
        int_forward
    >;
...
using enumerate_random =
    tested::ranges::enumerate_view<
        int_random_access
    >;

using enumerate_contiguous =
    tested::ranges::enumerate_view<
        int_contiguous
    >;
...
static_assert(
    exactly_forward_range<
        zip_transform_forward_value
    >
);

static_assert(
    exactly_random_access_range<
        zip_transform_random_value
    >
);

static_assert(
    exactly_random_access_range<
        zip_transform_random_reference
    >
);
...
using adjacent_random =
    tested::ranges::adjacent_view<
        int_random_access,
        2
    >;

using adjacent_contiguous =
    tested::ranges::adjacent_view<
        int_contiguous,
        2
    >;
...
static_assert(
    exactly_forward_range<
        adjacent_transform_forward_value
    >
);

static_assert(
    exactly_random_access_range<
        adjacent_transform_random_value
    >
);

static_assert(
    exactly_random_access_range<
        adjacent_transform_random_reference
    >
);
...
using slide_forward =
    tested::ranges::slide_view<
        int_forward
    >;

using slide_bidirectional =
    tested::ranges::slide_view<
        int_bidirectional
    >;
...
using single_pattern =
    tested::ranges::single_view<int>;

using split_forward =
    tested::ranges::split_view<
        int_forward,
        single_pattern
    >;
...
using join_input_outer =
    input_view<int_forward>;

using join_forward_outer =
    forward_view<int_forward>;

using join_bidirectional_outer =
    bidirectional_view<int_bidirectional>;

using join_random_outer =
    random_access_view<int_random_access>;

using join_input =
    tested::ranges::join_view<
        join_input_outer
    >;
...
using join_bidirectional =
    tested::ranges::join_view<
        join_bidirectional_outer
    >;
...
using cartesian_input =
    tested::ranges::cartesian_product_view<
        int_input,
        int_forward
    >;
