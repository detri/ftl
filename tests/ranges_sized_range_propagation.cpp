...
template<class T>
struct sized_non_common_range {
    T* first = nullptr;
    T* last = nullptr;

    constexpr sized_non_common_range() = default;

    constexpr sized_non_common_range(
        T* first,
        T* last
    ) noexcept
        : first(first),
...
template<class T>
struct sized_non_common_view
    : sized_non_common_range<T>,
      tested::ranges::view_base
{
    constexpr sized_non_common_view() = default;

    constexpr sized_non_common_view(
        T* first,
        T* last
    ) noexcept
        : sized_non_common_range<T>(
...
struct disabled_sized_range {
    ...
    constexpr unsized_sentinel<int>
    end() const noexcept {
        return {last};
    }
...
public:
    ...
    constexpr explicit input_iterator(
        int* current
    ) noexcept
        : current_(current) {}
    ...
    : tested::ranges::view_base
{
    ...
    constexpr input_iterator begin() noexcept {
        return input_iterator(first);
    }

    constexpr input_iterator end() noexcept {
        return input_iterator(last);
    }
};
...
concept sized_mutable_and_const =
    ...
    tested::ranges::range<const R> &&
    tested::ranges::sized_range<R> &&
...
using sized_ref_view =
    tested::ranges::ref_view<int[5]>;

using sized_owning_view =
    tested::ranges::owning_view<
        sized_non_common_range<int>
    >;

using unsized_owning_view =
    tested::ranges::owning_view<
        unsized_range<int>
    >;

using unsized_subrange =
    tested::ranges::subrange<
        int*,
        unsized_sentinel<int>
    >;

static_assert(
    sized_mutable_and_const<
        sized_ref_view
    >
);
...
sized_range_concepts_work() {
    ...
    tested::pair<int, int> pairs[] = {
        {1, 10},
        {2, 20},
        {3, 30}
    };

    sized_non_common_view<int>
        sized_non_common{
            values,
            values + 5
        };

    unsized_view<int> unsized{
        values,
        values + 5
    };
    ...
    auto unsized_elements =
        unsized_pairs |
        tested::ranges::views::
            elements<0>;

    auto unsized_as_const =
        tested::ranges::views::as_const(
            unsized
        );
    ...
    auto unsized_enumerate =
        tested::ranges::views::enumerate(
            unsized
        );

    auto unsized_adjacent =
        unsized |
        tested::ranges::views::
            adjacent<2>;
    ...
    auto unsized_split =
        unsized |
        tested::ranges::views::split(3);

    auto unsized_lazy_split =
        unsized |
        tested::ranges::views::
            lazy_split(3);
    ...
    static_assert(
        correctly_unsized<
            decltype(unsized_take)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_drop)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_transform)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_common)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_elements)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_as_const)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_stride)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_enumerate)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_adjacent)
        >
    );
    ...
    static_assert(
        correctly_unsized<
            decltype(unsized_chunk)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_filter)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_take_while)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_drop_while)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_split)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_lazy_split)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_chunk_by)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_zip)
        >
    );
    ...
    static_assert(
        correctly_unsized<
            decltype(unsized_product)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(unsized_join)
        >
    );

    static_assert(
        correctly_unsized<
            decltype(input_chunk)
        >
    );
...
sized_range_values_work() {
    ...
    auto take_too_many =
        values |
        tested::ranges::views::take(8);

    auto take_none =
        values |
        tested::ranges::views::take(0);
