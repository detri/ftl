...
concept has_plus_assign =
    requires(
        T value,
        typename T::difference_type offset
    ) {
    value += offset;
    };
...
struct input_test_sentinel {
    ...
    friend constexpr bool operator==(
        const input_test_iterator& iterator,
        input_test_sentinel sentinel
    ) noexcept {
...
static_assert(
    tested::input_iterator<input_test_iterator>
);

static_assert(tested::sentinel_for<
    input_test_sentinel,
    input_test_iterator
>);

static_assert(!tested::sized_sentinel_for<
    input_test_sentinel,
    input_test_iterator
>);

using input_const_iterator =
    tested::basic_const_iterator<
        input_test_iterator
    >;

static_assert(tested::same_as<
    typename mutable_iterator::iterator_concept,
    tested::contiguous_iterator_tag
>);

static_assert(tested::same_as<
    typename mutable_iterator::iterator_category,
    tested::random_access_iterator_tag
>);

static_assert(tested::same_as<
    typename mutable_iterator::value_type,
    int
>);

static_assert(tested::same_as<
    typename mutable_iterator::difference_type,
    tested::ptrdiff_t
>);
...
static_assert(tested::same_as<
    decltype(
        tested::declval<
            const mutable_iterator&>().operator->()
    ),
    const int*
>);

static_assert(tested::same_as<
    decltype(tested::ranges::iter_move(
...
static_assert(tested::same_as<
    tested::common_reference_t<const int&&, int&&>,
    const int&&
>);

static_assert(tested::same_as<
    tested::common_reference_t<int&&, const int&&>,
    const int&&
>);

static_assert(tested::same_as<
    tested::common_reference_t<int&&, int&&>,
    int&&
>);
...
static_assert(requires(
    mutable_iterator mutable_value,
    constant_iterator constant_value,
    int* mutable_pointer,
    const int* constant_pointer
) {
    { mutable_value == mutable_pointer }
        -> tested::same_as<bool>;

    { mutable_pointer == mutable_value }
        -> tested::same_as<bool>;
    ...
    { constant_value > mutable_value }
        -> tested::same_as<bool>;

    mutable_value <=> constant_value;

    { mutable_value - constant_value }
        -> tested::same_as<tested::ptrdiff_t>;

    { constant_pointer - mutable_value }
        -> tested::same_as<tested::ptrdiff_t>;
});
 ...
 */
static_assert(
    !has_iterator_category<input_const_iterator>
);
 ...
 */
static_assert(
    !has_predecrement<input_const_iterator>
);

static_assert(
    !has_plus_assign<input_const_iterator>
);

static_assert(
    !has_subscript<input_const_iterator>
);
...
static_assert(
    has_predecrement<mutable_iterator>
);

static_assert(
    has_plus_assign<mutable_iterator>
);

static_assert(
    has_subscript<mutable_iterator>
);
...
struct throwing_move_iterator {
    ...
    constexpr throwing_move_iterator&
    operator++() noexcept {
        ++current;
        return *this;
    }
    ...
    friend constexpr bool operator==(
        const throwing_move_iterator&,
        const throwing_move_iterator&
    ) = default;

    friend constexpr int&& iter_move(
        const throwing_move_iterator& iterator
    ) noexcept(false) {
...
bool ftl_test() {
    return basic_const_iterator_works();
}
