...
bool multibyte_locale_routing_works()
{
    char saved_locale[128]{};

    const char* current =
            tested::setlocale(LC_CTYPE, nullptr);

    if (current == nullptr)
        return false;

    tested::size_t saved_length = 0;

    while (current[saved_length] != '\0')
    {
        if (saved_length + 1 == sizeof(saved_locale))
            return false;

        saved_locale[saved_length] = current[saved_length];
        ++saved_length;
    }

    saved_locale[saved_length] = '\0';
    ...
    struct locale_restore
    {
        ...
        {
            static_cast<void>(
                tested::setlocale(LC_CTYPE, name));
        }
    } restore{saved_locale};

#if defined(_WIN32)
    const char* utf8_locale = ".UTF-8";
#elif defined(__APPLE__)
    const char* utf8_locale = "en_US.UTF-8";
#else
    ...
    // Restartable mbrtowc across unrelated source buffers.
    {
        tested::mbstate_t state{};
        wchar_t output{};

        if (tested::mbrtowc(
                &output, cent, 1, &state) != incomplete)
        {
            return false;
        }
    ...
    // Sizing pass must not move source.
    {
        tested::mbstate_t state{};
        const char* source = cent;

        if (tested::mbsrtowcs(
                nullptr, &source, 0, &state) != 1)
        {
            return false;
        }
    ...
    // Wide character -> current UTF-8 C locale.
    {
        ...
        const auto count =
                tested::wcrtomb(output, L'\u00a2', &state);

        if (count != 2 ||
            static_cast<unsigned char>(output[0]) != 0xc2 ||
    ...
    // Wide string -> multibyte string.
    {
        ...
        if (tested::wcsrtombs(
                output, &source, sizeof(output), &state) != 2)
        {
            return false;
        }
    ...
    // Wide stdio must use the locale snapshot/state path as well.
    {
        ...
        if (tested::fputwc(L'\u00a2', file) == WEOF)
            return false;

        if (tested::fflush(file) != 0)
            return false;

        tested::rewind(file);

        if (tested::fgetwc(file) !=
            static_cast<tested::wint_t>(L'\u00a2'))
        {
            return false;
        }
    }
...
bool ftl_test()
{
    ...
    if (!multibyte_locale_routing_works())
        return false;
