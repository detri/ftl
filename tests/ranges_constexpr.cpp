    ...
    : tested::ranges::view_base
{
    ...
    constexpr input_sentinel end() noexcept
    {
    ...
    : tested::ranges::view_base
{
    ...
    constexpr sized_sentinel end() const noexcept
    {
        return sized_sentinel{
            last
        };
    }
    ...
    : tested::ranges::view_base
{
    ...
    constexpr inner_view* begin() const noexcept
    {
        return first;
    }

    constexpr inner_view* end() const noexcept
    {
        return last;
    }
};
...
constexpr bool foundational_views_work()
{
    ...
    if (
        reference.begin() != values ||
        reference.end() != values + 4 ||
        reference.size() != 4 ||
        reference.front() != 1 ||
        reference.back() != 4 ||
        reference[2] != 3
    ...
    tested::ranges::subrange subrange(
        values,
        values + 4
    );
...
constexpr bool classic_views_work()
{
    ...
    const auto& constant_transformed =
        transformed;

    if (
        constant_transformed[1] != 4 ||
        constant_transformed.end() -
            constant_transformed.begin() != 6
    )
    {
        return false;
    }
    ...
    auto taken_past_end =
        base |
        tested::ranges::views::take(20);

    if (
        taken_past_end.size() != 6 ||
        taken_past_end.back() != 6
    )
    {
        return false;
    }
...
constexpr bool generated_views_work()
{
    ...
    auto unbounded =
        tested::ranges::views::iota(8);

    auto unbounded_iterator =
        unbounded.begin();

    unbounded_iterator += 20;

    if (
        *unbounded_iterator != 28 ||
        unbounded_iterator ==
            unbounded.end()
    )
    {
...
constexpr bool multi_range_views_work()
{
    ...
    const auto& constant_product =
        product;

    auto constant_value =
        constant_product.begin()[2];
...
constexpr bool window_views_work()
{
    ...
    const int second_group[] = {
        7,
        8
    };

    const int third_group[] = {
        20
    };
    ...
    if (
        chunked.size() != 3 ||
        sum_range(chunked[0]) != 3 ||
        sum_range(chunked[1]) != 12 ||
        sum_range(chunked[2]) != 5
    )
    {
        return false;
    }
...
constexpr bool cached_paths_work()
{
    ...
    auto first_filter_begin =
        filtered.begin();

    if (
        *first_filter_begin != 2 ||
        filter_calls == 0
    )
    {
        return false;
    ...
    const int calls_after_filter_begin =
        filter_calls;

    auto second_filter_begin =
        filtered.begin();
    ...
    const int calls_after_drop_begin =
        drop_calls;

    auto second_drop_begin =
        dropped.begin();
    ...
    auto first_reverse_begin =
        reversed.begin();

    if (
        *first_reverse_begin != 4 ||
        comparisons == 0
    )
    {
        return false;
    ...
    const int comparisons_after_begin =
        comparisons;

    auto second_reverse_begin =
        reversed.begin();

    if (
        second_reverse_begin !=
            first_reverse_begin ||
        comparisons !=
            comparisons_after_begin
    )
    {
        return false;
    }
...
static_assert(
    generated_views_work()
);

static_assert(
    multi_range_views_work()
);
