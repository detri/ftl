...
bool uncaught_count_works() {
  struct observer {
    bool *result;
    ~observer() { *result = tested::uncaught_exceptions() > 0; }
  };
...
bool inaccessible_and_ambiguous_nested_are_noops() {
  ...
  ambiguous_nested ambiguous_value;

  tested::rethrow_if_nested(private_value);
  tested::rethrow_if_nested(ambiguous_value);
