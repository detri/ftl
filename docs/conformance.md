...
| `<filesystem>`                                                | BLOCKED       | Path source constraints, root/trailing-separator decomposition, iterator-provided attribute caching without indirect `refresh()`, observable `directory_entry::refresh` updates, and missing-source `copy`/`copy_file` error reporting are remediated with regressions. Native Windows operations already preserve Unicode through the UTF-8-to-wide runtime boundary. Full Windows synopsis conformance is blocked by the existing public UTF-8 path ABI: Windows requires `path::value_type == wchar_t`, `wstring_type`, and `preferred_separator == L'\\'`; changing that representation requires a coordinated ABI break across filesystem, file streams, and the native runtime. |
