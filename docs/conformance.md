...
| `<cmath>` core                                                | PARTIAL       | `float_t`/`double_t` follow `FLT_EVAL_METHOD`; common floating argument selection uses compiler conversion rank/subrank; constexpr remainder reduction avoids overflowing `x / y`; sign inspection is endian-safe; quiet comparisons avoid invalid exceptions; and standard `_Float128` square root preserves its type. AppleClang and Clang before 21 lack a constant-evaluable fused builtin and use the unfused constexpr FMA fallback. Remaining extended-type runtime dispatch is open. |
| `<cmath>` mathematical special functions                      | PARTIAL       | `beta` uses scaled log-domain evaluation that avoids overflowing `x + y` and individual log-gamma terms. Focused accuracy and full GCC/Clang normal/replacement suites pass; extended floating/integer overload dispatch remains tied to the unresolved core extended-floating runtime boundary.                                                                                                                                                                                                         |
...
| `<chrono>` formatting                                         | PARTIAL       | The chrono-spec grammar accepts leading/interspersed literals, extended floating duration precision inherits the repaired trait, and localized `%p`/`%r` dispatch through the formatting locale's `time_put` facet, with normal/replacement regressions. Full `E`/`O` alternative formatting remains tied to named-time integration; direct-output quality work remains open.                                                                                                                           |
