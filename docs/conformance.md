...
| `<cwchar>`                                                    | BLOCKED       | `WEOF` has exactly type `wint_t`, with normal/replacement synopsis regressions. Locale-sensitive multibyte conversion and wide I/O are blocked on the roadmap's locale-runtime boundary: FTL owns a stable `mbstate_t`, while host codec APIs require an incompatible opaque host state, and an internal UTF-8-only codec cannot implement the selected C locale. Parsing/collation/time formatting inherit the same missing codec/locale-data boundary. |
| `<cuchar>`                                                    | BLOCKED       | Locale-sensitive Unicode conversion and staged UTF-8/UTF-16 output are blocked on the same locale-runtime boundary as `<cwchar>`: FTL-owned `mbstate_t` cannot be passed to host codec APIs, while an internal UTF-8-only implementation would not satisfy arbitrary selected C locales. Existing C-locale state/error behavior remains covered in normal and replacement modes.                                                                                                                         |
...
| `<memory>` utilities, allocators, and ownership               | BLOCKED       | Historical lifetime, fancy-pointer, ownership conversion/order/cleanup, and allocator-backed array findings are repaired with normal/replacement regressions. Nontrivial constant-evaluated allocation works in replacement mode; normal-mode `ftl::allocator` is blocked by the C++23 language rule granting direct allocation-function constant evaluation specifically to `std::allocator`, which supported compilers do not extend to the parallel `ftl` namespace. |
...
| `<cstdlib>` / `<stdlib.h>`                                    | REMEDIATED    | `<cstdlib>` owns the floating/integer `abs` overloads and required termination/registration `noexcept` types. Namespace `bsearch`/`qsort` execute comparator calls through C++ linkage so exceptions propagate under MSVC `/EHsc`; focused synopsis, sorting, and exception regressions pass in normal and replacement modes. |
...
| `<complex>`                                                   | BLOCKED       | Converting assignment, stream insertion, and integer additional-overload participation are closed. Extended floating-point specializations are blocked at the runtime ABI boundary: the supported platform math runtimes expose complex transcendental/projection/power entry points only for `float`, `double`, and `long double`, with no portable ABI for the implementation-defined extended types. |
| `<random>`                                                    | REMEDIATED    | `generate_canonical` cannot round to `1`, and seed-like arguments convertible to an engine result type select value seeding. Direct ISO 28.5.3.1 re-check withdraws the extended-type findings: `RealType`, `IntType`, and `UIntType` are explicitly restricted to listed standard types, with other instantiations undefined; non-seed-sequence `Sseq` arguments are likewise outside the specified domain. Regressions cover all mandated 10,000th engine values, seed vectors, adaptors, state I/O, and distributions in both modes. |
| `<valarray>`                                                  | REMEDIATED    | Recorded construction rollback, element lifetime, empty-range access, non-default value construction, and full-domain `shift`/`cshift` defects are repaired. Focused normal/replacement tests and freestanding replacement linkage pass with MSVC, Clang-CL, GCC, and Clang; the permitted expression/proxy optimization remains a non-normative quality observation.                                                                                                                                  |
...
| `<cmath>` core                                                | BLOCKED       | `float_t`/`double_t`, conversion rank/subrank, constexpr reduction, sign classification, quiet comparisons, and `_Float128` square root are repaired. Complete extended-floating runtime dispatch is blocked because the supported platform C math ABIs do not provide a common callable surface for every implementation-defined extended type; narrowing through `long double` would violate the required return-domain semantics. |
| `<cmath>` mathematical special functions                      | BLOCKED       | `beta` uses scaled log-domain evaluation and the standard-type special-function surface is covered in both modes. Complete extended-floating dispatch is blocked by the same absent cross-platform runtime ABI as the core math functions; routing those overloads through `long double` would lose the extended domain and precision. |
...
| `<chrono>` formatting                                         | BLOCKED       | Chrono-spec literals, duration precision, and localized `%p`/`%r` facet dispatch are repaired with normal/replacement regressions. Full named-locale `E`/`O` alternative formatting is blocked because the locale runtime exposes no cross-platform era, alternative-digit, or composite time-pattern data; buffered direct output is a quality issue rather than a remaining semantic defect. |
| `<chrono>` parsing                                            | BLOCKED       | `from_stream` implements the calendar, clock, duration, zone, redundant-field, leap-second, transactional-failure, and stream-state paths, including `badbit` translation and conditional original rethrow for stream-buffer failures. Named-locale `%c`/`%x`/`%X`/`%r` and `E`/`O` alternatives are blocked because the locale runtime exposes localized names but no platform composite/alternative time grammar data. |
| `<locale>` localization library                               | BLOCKED       | Global locale publication is serialized; time-name parsing is case-insensitive; monetary grouping and unbounded conversion are repaired; and deprecated conversion adapters own their facets and preserve conversion state/counts. Named `time_get`/`time_put` composite and alternative representations are blocked because the locale runtime has no cross-platform source for locale date/time patterns or alternative-digit/era data. |
...
| `<istream>` / `<ostream>`                                     | REMEDIATED    | Formatted and unformatted input now translate stream-buffer, facet, whitespace-sentry, positioning, and synchronization exceptions to `badbit` and conditionally rethrow the original exception; output paths do likewise, while stream-buffer source extraction uses the specified `failbit` rule. Normal/replacement GCC and Clang regression matrices and full suites pass.                                                                                                                               |
| `<iomanip>`                                                   | BLOCKED       | Monetary truncation/grouping and `get_money`/`put_money`/`get_time`/`put_time` stream-state exception handling are repaired with normal/replacement regressions. Full named-locale time manipulator certification is blocked by the locale runtime's missing composite/alternative time-pattern data. |
...
| `<sstream>`                                                   | REMEDIATED    | String-buffer seek arithmetic is checked before addition, large put positions are restored without narrowing to `int`, locale exchange inherits the repaired `basic_streambuf::swap`, and inherited formatted/unformatted stream exception state is repaired. Normal/replacement GCC and Clang regressions and full suites pass.                                                                                                                                                                          |
| `<strstream>`                                                 | REMEDIATED    | The get area exposes characters appended through the put sequence, external put areas begin at the supplied `pbeg`, large put offsets advance without narrowing, and seek arithmetic is overflow-safe. Owned buffers retain paired allocation/deallocation callbacks and freezing behavior; focused and full GCC/Clang normal/replacement suites pass.                                                                                                                                                  |
| `<spanstream>`                                                | REMEDIATED    | Span-buffer seek arithmetic is checked before addition, large put positions are restored without narrowing to `int`, buffer locale exchange inherits the repaired base swap, and inherited formatted/unformatted stream exception state is repaired. Normal/replacement GCC and Clang regressions and full suites pass.                                                                                                                                                                                  |
| `<fstream>`                                                   | REMEDIATED    | File-buffer positioning uses the runtime's full 64-bit native offset instead of narrowing through Windows `long`; input/output transitions perform the required flush or positioning operation and preserve unread external sequences/conversion state; wide multibyte extraction and putback retain external-width state; inherited stream exception handling is repaired. Sparse large-position, bidirectional transition, and deterministic multibyte regressions pass in normal/replacement GCC and Clang suites. |
| `<syncstream>`                                                | REMEDIATED    | `emit_on_flush`, `noemit_on_flush`, and `flush_emit` dispatch through allocator-independent stream-buffer hooks; `osyncstream::emit` follows unformatted-output sentry, `badbit`, and conditional-original-rethrow rules; stream-buffer locale swap is preserved; and fixed lock striping replaces the leaking per-address registry while preserving total-order synchronization. Normal/replacement GCC and Clang regressions and full suites pass.                                                   |
| `<iostream>`                                                  | BLOCKED       | Standard narrow/wide buffers share locked C-runtime stream state; wide buffers perform orientation-time locale conversion, UTF-8 multibyte I/O and pushback, and sequence writes under one file lock; inherited exception state is repaired. Certification for state-dependent named encodings is blocked by the Windows CRT's locale-specific conversion ABI, which provides only non-restartable `_mbtowc_l`/`_wctomb_l` operations and no per-stream `mbstate_t` interface. |
| `<filesystem>`                                                | BLOCKED       | Path source constructors now reject non-encoding character sources; decomposition edge cases, refreshed `directory_entry` caches, and missing-source `copy`/`copy_file` error reporting have regression coverage. Native operations preserve Unicode on Windows through the UTF-8-to-Win32-wide runtime boundary. Full Windows synopsis conformance remains blocked by the existing public UTF-8 `path` ABI: Windows requires `path::value_type == wchar_t`, `wstring_type`, and `preferred_separator == L'\\'`; changing that representation requires a coordinated ABI break across filesystem, file streams, and the native runtime. |
| `<cstdio>` / `<stdio.h>`                                      | BLOCKED       | Floating formatting/scanning honors the current radix, high precision uses freestanding C scratch allocation, normal/replacement stream macros are present without hijacking pre-existing vendor macros, and wide streams snapshot their orientation-time locale with UTF-8 conversion, pushback, and `fpos_t` state/position preservation. Stateful named encodings remain blocked because Windows exposes no restartable locale-specific conversion API from which to maintain the required per-stream `mbstate_t`. |
