...
| `<exception>` | PARTIAL | Implementation and tests changed, but this audit unit was not included in the completed remediation claim; reconcile before closure. |
...
| `<cwchar>` | OPEN | Historical findings not yet reconciled against this working tree. |
| `<cuchar>` | OPEN | Historical findings not yet reconciled against this working tree. |
...
| `<flat_set>` | PARTIAL | Bulk merge complexity, concrete assignment, ordering fallback, and allocator detection implemented; targeted tests, guide matrix audit, and non-MSVC validation remain. |
| `<flat_map>` | PARTIAL | Bulk merge complexity, concrete assignment, ordering fallback, and allocator detection implemented; targeted tests, guide matrix audit, and non-MSVC validation remain. |
| `<span>` | CERTIFIED | Historically certified; final dependency-invalidation pass pending. |
| `<mdspan>` | OPEN | Historical findings not yet reconciled against this working tree. |
...
| `<numeric>` | OPEN | Historical findings remain: operation ordering/constraints, scan and reduction behavior, and related synopsis requirements must be reconciled and fixed. |
| `<memory>` utilities, allocators, and ownership | OPEN | Historical allocator, pointer utility, ownership, construction, and synopsis findings have not been reconciled. |
| `<memory>` specialized algorithms | OPEN | Historical uninitialized-memory and destruction algorithm findings have not been reconciled. |
...
| `<cfenv>` / `<fenv.h>` | OPEN | Historical floating-point environment declarations and behavior findings remain. |
| `<complex>` | OPEN | Historical constructor, operator, function, branch-cut, and numerical-behavior findings remain. |
| `<random>` | OPEN | Historical engine, distribution, seeding, serialization, constraints, and numerical requirements remain. |
| `<valarray>` | OPEN | Historical expression, slice, operation, overload, and complexity findings remain. |
| `<numbers>` | OPEN | Historical constants, types, precision, and feature-test requirements remain. |
| `<cmath>` core | OPEN | Historical classification, comparison, overload, promotion, error, and mathematical behavior findings remain. |
| `<cmath>` mathematical special functions | OPEN | Historical special-function availability, overload, domain, error, and accuracy findings remain. |
| `<chrono>` durations, time points, and clocks | OPEN | Historical arithmetic, conversion, rounding, clock, constraint, and synopsis findings remain. |
| `<chrono>` civil calendar and `hh_mm_ss` | OPEN | Historical calendar types, conversions, validation, arithmetic, and `hh_mm_ss` findings remain. |
| `<chrono>` UTC/TAI/GPS/file clocks and clock conversion | OPEN | Historical clock types, epochs, conversions, leap-second handling, and `clock_cast` findings remain. |
| `<chrono>` time-zone database and `zoned_time` | OPEN | Historical tzdb, zone lookup, local-time resolution, exception, and `zoned_time` findings remain. |
| `<chrono>` formatting | OPEN | Historical chrono formatter grammar, localization, output, and participation findings remain. |
| `<chrono>` parsing | OPEN | Historical `from_stream`, parse grammar, state, localization, and error findings remain. |
| `<locale>` localization library | OPEN | Historical locale, facet, category, collation, conversion, numeric, monetary, and time findings remain. |
...
The working tree intentionally remains uncommitted. It contains the complete
remediation accumulated in this session plus this copied ledger. Preserve all
of it; the changes are interdependent and were made on top of pre-existing
work on the active branch.

Recommended continuation order:

1. Finish `<flat_set>` and `<flat_map>` before moving to later ledger sections.
   Add tests that count comparisons for both general and sorted-tag insertion,
   assert concrete initializer-list assignment return types, exercise a type
   with only `==` and `<`, and cover every allocator-bearing deduction guide.
2. Validate those focused targets in the canonical MSVC and Clang-CL folders,
   then in the canonical WSL GCC and Clang folders, in both normal and
   replacement modes. The existing MSVC focused targets are
   `ftl_normal_flat_set`, `ftl_normal_flat_map`, `ftl_replace_flat_set`, and
   `ftl_replace_flat_map`.
3. Continue through the remaining `FAILED` audit records in ledger order.
   Keep the two known platform/library blockers visible: hosted MSVC
   `<typeinfo>` exception-object identity, and locale-sensitive floating
   conversions in `<string>`.
4. Reconcile every historical defect below against the final working tree.
   Only then change individual ledger statuses. The progress list above is
   implementation evidence, not certification.
5. After focused work is stable, run the complete suites once in
   `build-string-msvc-release`, `cmake-build-relwithdebinfo-vs-clang`,
   `build-string-gcc-wsl`, and `build-string-clang-wsl`, with both normal and
   `FTL_REPLACE_STL` coverage. Also run freestanding/public-header closure as
   required by `AGENTS.md` and finish with `git diff --check`.

Validation completed immediately before this handoff:

```text
MSVC x64 Release build: flat_set/flat_map, normal + replacement: PASS
MSVC x64 Release execution: flat_set/flat_map, normal + replacement: PASS
git diff --check: PASS
Full supported compiler matrix: NOT RUN
Full test suites: NOT RUN
Ledger re-audit/certification: NOT RUN
```
