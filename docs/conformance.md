...
| `<memory>` utilities, allocators, and ownership               | PARTIAL       | Historical lifetime, fancy-pointer, ownership conversion/order/cleanup, and allocator-backed array findings are repaired with focused regressions. Nontrivial constant-evaluated allocation works in replacement mode; normal-mode `ftl::allocator` remains blocked because C++23 grants direct allocation-function constant evaluation specifically to `std::allocator`, and supported compilers do not extend that privilege to the parallel `ftl` namespace. |
