...
| `<locale>` localization library                               | PARTIAL       | Global locale publication is serialized; time-name parsing is case-insensitive; monetary grouping and unbounded conversion are repaired; and the required C++23 deprecated `wstring_convert`/`wbuffer_convert` surfaces own their facets and preserve conversion state/counts while adapting FTL stream buffers. Focused and full GCC/Clang normal/replacement suites pass; named composite/alternative time forms remain open.                                                                        |
